...
const adminController = {
		...
		try {
			...
			if (mainRecords.length === 0) {
				return res.status(200).json({
					message: "Записи 'Главная' не найдены",
					isCreated: false,
				});
			}
			...
			console.log(error);
			res.status(500).json({
				error: "Ошибка при получении списка записей 'Главная'",
			});
		}
	},
	...
	deleteCategory: async (req, res) => {
		try {
			...
			if (categoryResult.length === 0) {
				return res
					.status(404)
					.json({ message: "Категория не найдена" });
			}
	...
	deleteBanner: async (req, res) => {
		try {
			...
			const bannerResult =
				await sql`SELECT * FROM banners WHERE id = ${id}`;
