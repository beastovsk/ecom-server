...
const blogController = {
    createBlog: async (req, res) => {
        try {
            ...
            const result = await sql`
                INSERT INTO blogs (title, images, content)
                VALUES (${title}, ${imageJson}::jsonb, ${content})
                RETURNING *`;
    ...
    getAllBlogs: async (req, res) => {
        try {
            ...
            if (result.length === 0) {
                return res.status(200).json({ message: "Блоги не найдены" });
            }
    
            res.json({ blogs: result });
        } catch (error) {
            console.log(error);
            res.status(500).json({ error: "Ошибка при получении списка блогов" });
        }
    },
